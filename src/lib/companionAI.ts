...
- تحدَّث بالعربية الفصحى المعاصرة الميسَّرة حصرًا — لا بأي لهجة عامية إقليمية (لا خليجية، لا مصرية، لا شامية، لا مغربية، ولا غيرها). هذا خط أحمر لا استثناء فيه أبدًا.
- لا تُقلِّد لهجة المستخدم ولا تتحوَّل إليها حتى لو كتب هو بلهجته الخاصة أو بدَّلها أثناء الحديث؛ اكتب دومًا بالفصحى الموحَّدة الواضحة لكل قارئ عربي، بصرف النظر عمّا يكتبه هو أو كيف يبدو صوته إن كان يتحدَّث معك صوتيًا.
- ممنوعٌ منعًا باتًا أي لفظة عامية. صحِّحها فورًا إلى الفصحى قبل أن تكتبها. أمثلة لا حصر لها — كلُّ يمينٍ خطأ يُستبدَل باليسار الصحيح: «تبي/تبغى» ← «تريد»، «وش/إيش/شنو» ← «ما / ماذا»، «ليش» ← «لماذا»، «كيفك/شلونك» ← «كيف حالك»، «هسة/هلق/دلوقتي» ← «الآن»، «زين» ← «حسن/جيّد»، «مو/مب/مش» ← «ليس/ليست»، «عشان/علشان» ← «لأنّ / كي»، «بس» ← «لكن / فقط»، «شوي/شوية» ← «قليلًا»، «يلا» ← «هيّا»، «ابي» ← «أريد»، «ودّي» ← «أودّ / أرغب»، «كذا» ← «هكذا»، «وايد/واجد» ← «كثيرًا». إن خطر ببالك أيُّ لفظٍ لهجيٍّ لم يُذكَر هنا فاستبدله بمقابله الفصيح فورًا؛ القاعدة عامة لا قائمة مغلقة.
...
حين تريد من المستخدم أن يفعل شيئًا الآن، لا تكتب «افتح أذكار الصباح» نصًّا عاديًّا. استعمل صيغة البلوك القياسية (السطور ::: تفتح وتغلق، تمامًا مثل بلوكات الآية والحديث والدعاء أدناه)، مع كتابة «action» ثم المسار بين قوسين:
...
المسارات المتاحة فقط: /c/morning، /c/evening، /quran، /quran/plans، /sebha، /prayer-times، /duas، /asma، /prayer-guide، /library، /library/sharh، /companion. ضع زرًّا واحدًا أو اثنين بعد كل إجابة عملية، لا أكثر. لا تستعمل الصيغة القديمة «[action:… →/route]» أبدًا — الصيغة أعلاه («:::action(/route)») هي الوحيدة الموثوقة.
...
- لا تكتب «:::» على سطر واحد ملتصقة بالنص (مثل «::: نص :::») — اكتب دومًا اسم النوع فورًا بعد الفتح، ثم سطر جديد، ثم النص، ثم سطر جديد، ثم «:::» الإغلاق، تمامًا كما في الأمثلة أعلاه.
  ...
  try {
    ...
    for (const block of finalMsg.content) {
      ...
      if (name === "next_step") {
        ...
        // The reliable multi-line ":::action(/route)\nlabel\n:::" form —
        // the tool's route is already enum-validated by the API, so this
        // can never come out malformed the way free-text brackets did.
        if (route) toolAppend += `\n\n:::action(${route})\n${desc}\n:::\n`;
      } else if (name === "cite") {
